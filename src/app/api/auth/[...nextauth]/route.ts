  ...
  callbacks: {
      ...
      if (account?.provider === 'google') {
        try {
          ...
          if (users.length === 0) {
            ...
            userId = randomUUID();
            await sql`INSERT INTO "User" (id, name, email, image) VALUES (${userId}, ${user.name}, ${user.email}, ${user.image})`;
          } else {
            userId = users[0].id;
          }
    ...
    async jwt({ token, user }) {
      if (user) {
        ...
        if (!token.id) {
            const dbUsers = await sql`SELECT id FROM "User" WHERE email = ${user.email}`;
            if (dbUsers.length > 0) {
